...
for exp_idx in tqdm(range(len(experiment_names))):
    ...
    try:
        ...
        _, lons, lats = DIC_location.T
        max_alphas[lons, lats] = max_alpha
...
p2.plot_surface2d(model_lon, model_lat, max_alphas, 0, 100, cmap, 'maximum cumulative additionality')
...
for t_idx in tqdm(range(0, 4)):
    p2.plot_surface2d(model_lon, model_lat, ds.delDIC.isel(time=t_idx, depth=0).values*-100, 0, 1, 'viridis', 'delDIC at t = ' + str(ds.time.isel(time=t_idx).values))
