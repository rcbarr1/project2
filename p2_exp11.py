...
for exp_idx in range(0, len(experiment_names)):
    ...
    del_q_CDR_DIC_3D[0, experiment_lon_idx, experiment_lat_idx] = -1e-6 # mol kg-1 yr-1
...
for idx in range(0, nt):
    del_C_CDR[idx] = np.nansum(del_q_CDR_DIC_3D * dt1 * idx * rho * model_vols) # [mol CO2]
    del_C_atm[idx] = data['delxCO2'].isel(time=idx).values * Ma * 1e-6 # [mol CO2]
